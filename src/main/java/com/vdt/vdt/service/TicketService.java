...
public class TicketService {
    ...
    public String completeTicket(Long ticketId) {
        ...
        ticket.setResolutionTimeMinutes(Duration.between(ticket.getCreatedAt(), LocalDateTime.now()).toMinutes());
        ticketRepository.save(ticket);
    ...
    public List<TicketDetailDto> getAllTicketsForCustomer(Long customerId) {
        ...
        return tickets.stream()
                .map(ticket -> ticketMapper.map(ticket, TicketDetailDto.class))
    ...
    public String getSlaBreachStatus(Long ticketId) {
        Optional<Ticket> ticketOpt = ticketRepository.findById(ticketId);
        if (ticketOpt.isPresent()) {
            Ticket ticket = ticketOpt.get();
            Duration remainingTime = getTimeLeftBeforeSlaBreach(ticket.getSlaResolutionDueAt());
            return formatRemainingTime(remainingTime); // Format duration to display (e.g., "2 hours 30 minutes")
        }
